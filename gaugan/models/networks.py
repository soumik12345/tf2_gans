...
def build_encoder(image_shape, encoder_downsample_factor=64, latent_dim=256):
    ...
    x = downsample_block(encoder_downsample_factor, 3, apply_norm=False)(input_image)
    x = downsample_block(2 * encoder_downsample_factor, 3)(x)
    x = downsample_block(4 * encoder_downsample_factor, 3)(x)
    x = downsample_block(8 * encoder_downsample_factor, 3)(x)
    x = downsample_block(8 * encoder_downsample_factor, 3)(x)
    x = layers.Flatten()(x)
...
def build_discriminator(image_shape, downsample_factor=64):
    ...
    x1 = downsample_block(downsample_factor, 4, apply_norm=False)(x)
    x2 = downsample_block(2 * downsample_factor, 4)(x1)
    x3 = downsample_block(4 * downsample_factor, 4)(x2)
    x4 = downsample_block(8 * downsample_factor, 4, strides=1)(x3)
