...
def build_generator(mask_shape, latent_dim: int, alpha: float) -> Model:
    latent = Input(shape=(latent_dim))
    mask = Input(shape=mask_shape)
    x = layers.Dense(16384)(latent)
    x = layers.Reshape((4, 4, 1024))(x)
    x = ResidualBlock(filters=1024, alpha=alpha)(x, mask)
    x = layers.UpSampling2D((2, 2))(x)
    x = ResidualBlock(filters=1024, alpha=alpha)(x, mask)
    x = layers.UpSampling2D((2, 2))(x)
    x = ResidualBlock(filters=1024, alpha=alpha)(x, mask)
    x = layers.UpSampling2D((2, 2))(x)
    x = ResidualBlock(filters=512, alpha=alpha)(x, mask)
    x = layers.UpSampling2D((2, 2))(x)
    x = ResidualBlock(filters=256, alpha=alpha)(x, mask)
    x = layers.UpSampling2D((2, 2))(x)
    x = ResidualBlock(filters=128, alpha=alpha)(x, mask)
    x = layers.UpSampling2D((2, 2))(x)
